			...
			<!-- Begin hideable MARTINGALE warning --><div id="martingale_warn" class="hidden" style="text-align:center;background:#FAD7D7;border:1px solid red;border-radius:6px">
			Warning, the Martingale simulator is intended SOLELY to dissuade traders from using this strategy! By including it as an option in the simulator tool, 
			we are IN NO WAY condoning the employment of it or any similar double-down-after-loss strategy. We believe this is the best possible martingale strategy and 
			(as you will see if you test it with the simulator) it is still garbage in comparison to the money management that we actually recommend! If you are still not 
			convinced, watch our videos on the subject :)
			</div><!-- End hideable MARTINGALE warning -->		
                        <div class="row">
                            <div class="3u">
                                <div class="form-group">
									...
									echo '>Never</option>';
									echo '<option value ="3"';
									if($simulate == "on" && $error == "" && $_POST["method"] == 3) { echo " SELECTED"; }
									echo '>Martingale</option>';
									echo '</select>';									
									?>
                                </div>
                            </div>
                            <div class="3u">
								...
								<label class="fff">
                                    ...
                                    <label>
                                        Win Rate:
                                       <a  bubbletooltip="Expected success rate of the indicators that you are using to trade binary options. For updated Win Rates for all of our recommended signal providers check out the 'Trading' tab."><?php echo $tooltip_icon; ?></a>
                                    </label>
                    ...
                    <div class="row">
                            ...
                            <div class="3u">&nbsp &nbsp</div>
                            <div class="3u">
                                <div class="form-group">
                                    &nbsp &nbsp
                                    &nbsp &nbsp
                                    <input type="checkbox" id="advanced_checkbox" name="advanced_checkbox">
                                    <label for="advanced_checkbox"><span>Use Advanced Options</span></label>
                                </div>
                            </div>
                        </div>

		<!-- BEGIN HIDDEN ADVANCED OPTIONS --><div id="advanced" class="hidden">
		<hr><h4 style="text-align:center">Advanced Options</h4><hr>
		<?php
		for( $x = 2; $x <= 4; $x++ ){
                    echo '<div class="row">';
                            echo '<div class="3u">';
                                echo '<div class="form-group">';
					echo '<label class="fff">';
                                     		echo '&nbsp;';
					echo '</label>';
                                    	echo '<label>';
						echo 'Signal ' . $x . ' Win Rate:';
                                     		echo '<a bubbletooltip="Signal ' . $x . ' Win Rate">' . $tooltip_icon . '</a>';
                                    	echo '</label>';
                                    	if($simulate == "on" && $error == "") { $sp = $_POST["stake_percent"]; } else { $sp = ""; }
                                    	echo '<input type="number" class="textbox_sml" id="spreadwr' . $x . '" name="spreadwr' . $x . '" min="0" max="100" step="1" value="'.$sp.'">';
			...
			echo '<div class="3u">';
					...
					echo '<label class="fff">';
                                	...
                                	echo '</label>';
                                	echo '<label>';
                                		echo 'Signal ' . $x . ' Percent Staked per Trade:';
                                		echo '<a bubbletooltips="Percent of balance to be traded for Signal ' . $x . '. This will be implemented base on the selection for Frequency of Stake Percentage Calculation. In general as the percent of balance being traded increases both the risk and expected return will increase as well.">' . $tooltip_icon . '</a>';
                                    	echo '</label>';
                                    	if($simulate == "on" && $error == "") { $sp = $_POST["stake_percent"]; } else { $sp = ""; }
                                	echo '<input type="number" class="textbox_sml" id="spreadstake' . $x . '" name="spreadstake' . $x . '" min="0" max="100" step="1" value="' .$sp. '" >';
					echo '<input type="text" value="%" readonly="true" class="textbox_relative"/>';
                                echo '</div>';
                            echo '</div>';
			...
			echo '<div class="3u">';
					...
					echo '<label class="fff">';
                                	...
                                	echo '</label>';
                                	echo '<label>';
                                		echo 'Signal ' . $x . ' Return on Investment:';
                                		echo '<a bubbletooltips="Return on investment for a winning trade with Signal ' . $x . '.">' . $tooltip_icon . '</a>';
                                    	echo '</label>';
                                    	if($simulate == "on" && $error == "") { $sp = $_POST["stake_percent"]; } else { $sp = ""; }
                                	echo '<input type="number" class="textbox_sml" id="spreadreturn' . $x . '" name="spreadreturn' . $x . '" min="0" max="100" step="1" value="' .$sp. '" >';
					echo '<input type="text" value="%" readonly="true" class="textbox_relative"/>';
                                echo '</div>';
                            echo '</div>';
			...
			echo '<div class="3u">';
					...
					echo '<label class="fff">';
                                	...
                                	echo '</label>';
                                	echo '<label>';
                                		echo 'Signal ' . $x . ' Percent of Total Signals:';
                                		echo '<a bubbletooltips="Percent of the total signals that Signal ' . $x . ' make up.">' . $tooltip_icon . '</a>';
                                    	echo '</label>';
                                    	if($simulate == "on" && $error == "") { $sp = $_POST["stake_percent"]; } else { $sp = ""; }
                                	echo '<input type="number" class="textbox_sml" id="spreadfreq' . $x . '" name="spreadfreq' . $x . '" min="0" max="100" step="1" value="' .$sp. '" >';
					echo '<input type="text" value="%" readonly="true" class="textbox_relative"/>';
                                echo '</div>';
                            echo '</div>';
                        echo '</div>';
		...
		?>
		</div><!-- END HIDDEN ADVANCED OPTIONS -->
						...
						<?php
						/*
						 *AVAILABLE VARIABLES (set in toolbox.php) --> now in pre-tools.php
						 *
						 *$r      = $_POST["return"]/100;
						...
						if($simulate == "on") 
						{
							if($error == "") {
								...
								if($_POST["method"] == 0) {
									if(isset($_POST['advanced_checkbox'])){
										$results = spread_simdaily(1500, $i, $days, $min, $max, $tpd, $ruin, $p, $p2, $p3, $p4, $t, $t2, $t3, $t4, $r, $r2, $r3, $r4, $sp2, $sp3, $sp4);
									}else{
									...
									$n = $tpd * $days;
									if(isset($_POST['advanced_checkbox'])){
										$results = spread_simulator(1500, $i, $n, $ruin, $t, $min, $max, $p, $p2, $p3, $p4, $t, $t2, $t3, $t4, $r, $r2, $r3, $r4, $sp2, $sp3, $sp4);
									}else{
										$results = simulator(1500, $i, $n, $ruin, $t, $min, $max, $p, $r);
									}
								}elseif($_Post["method"] == 2){
								if(isset($_POST['advanced_checkbox'])){
										$results = spread_flatsim($sims, $i, $days, $min, $max, $tradesperday, $ruin, $p, $p2, $p3, $p4, $t, $t2, $t3, $t4, $r, $r2, $r3, $r4, $sp2, $sp3, $sp4);
									}else{
										$results = flatsim(1500, $i, $days, $t, $min, $max, $tpd, $ruin, $p, $r);
									}
								}else{
									$results = martingale(1500, $i, $days, $tpd, $ruin, $t, $min, $max, $p, $r);
								}
								$divisor = 1;
